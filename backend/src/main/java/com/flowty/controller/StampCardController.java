...
public class StampCardController {
    ...
    @GetMapping
    public ResponseEntity<List<StampCardResponse>> getStampCards(
            @AuthenticationPrincipal UserDetails principal) {
        return ResponseEntity.ok(stampCardService.getUserStampCards(principal.getUsername()));
    }

    @GetMapping("/active")
    public ResponseEntity<StampCardResponse> getActiveCard(
            @AuthenticationPrincipal UserDetails principal) {
        return ResponseEntity.ok(stampCardService.getOrCreateActiveCard(principal.getUsername()));
    }

    @GetMapping("/transactions")
    public ResponseEntity<List<RewardTransactionResponse>> getRewardTransactions(
            @AuthenticationPrincipal UserDetails principal) {
        return ResponseEntity.ok(stampCardService.getUserRewardTransactions(principal.getUsername()));
    }
